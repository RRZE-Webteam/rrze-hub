...
define( 'HUBLOGFILE', plugin_dir_path( __FILE__) . 'rrze-faq-' . preg_replace( $s, $r,  get_bloginfo( 'url' ) ) . '.log' );
...
{
    load_plugin_textdomain('rrze-hub', false, sprintf('%s/languages/', dirname(plugin_basename(__FILE__))));
}
    ...
    if ($error = systemRequirements()) {
        add_action('admin_init', function () use ($error) {
            ...
            add_action($tag, function () use ($pluginName, $error) {
                printf(
                    '<div class="notice notice-error"><p>' . __('Plugins: %1$s: %2$s', 'rrze-hub') . '</p></div>',
                    esc_html($pluginName),
                    esc_html($error)
                );
            });
